...
// Documentation: https://github.com/KudoAI/bravegpt/tree/main/docs
...
(async () => {
    ...
    window.app = {
        ...
        commitHashes: {
            ...
            aiweb: 'db952f3' // for cached ai-chat-apis.json5 + <code-languages|katex-delimiters|sogou-tts-lang-codes>.json
        }
    }
            ...
            app.styles.textContent = (
             ...
             + `.no-user-select {
                ...
                #${app.slug}:has(.${app.slug}-alert) { /* app alerts */
                    border: var(--app-border) ; background-image: var(--app-gradient-bg) ; margin-bottom: 22px }
                ...
                #${app.slug}-header-btns {
                    display: flex ; direction: rtl ; gap: 2px ; float: right ; position: relative ; bottom: 3px }
                .${app.slug}-header-btn {
             ...
             + `#${app.slug} .feedback {
                ...
                #${app.slug} footer a:hover {
                    color: ${ appScheme == 'dark' ? 'white' : 'black' }; text-decoration: none }`
    ...
    window.toggle = {
        ...
        autoGen(mode) {
            const validModes = ['get', 'summarize'],
                  modeKey = `auto${string.toTitleCase(mode)}${ mode == 'get' ? 'Disabled' : '' }`
            ...
            if (settings.typeIsEnabled(modeKey)) { // this Auto-Gen mode toggled on, disable other one + Manual-Gen + do it
                const otherMode = validModes[+(mode == validModes[0])],
                      otherModeKey = `auto${string.toTitleCase(otherMode)}${ otherMode == 'get' ? 'Disabled' : '' }`
        ...
        manualGen(mode) { // Prefix/Suffix modes
            ...
            if (app.config[modeKey]) // Manual-Gen toggled on, disable all Auto-Gen
                ['get', 'summarize'].forEach(mode => {
                    if (settings.typeIsEnabled(`auto${string.toTitleCase(mode)}${ mode == 'get' ? 'Disabled' : '' }`)) {
        ...
        reply({ content, standby = false, apiUsed = null }) {
            ...
            if (!app.div.querySelector('.reply-pre')) {
                ...
                const appHeaderDiv = dom.create.elem('div',
                    { class: 'app-header', style: 'height: auto ; min-height: 36px ; margin: -8px 0' })
